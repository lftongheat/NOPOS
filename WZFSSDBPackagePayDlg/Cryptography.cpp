// Cryptography.cpp: implementation of the CCryptography class.
//
//////////////////////////////////////////////////////////////////////
...
void CCryptography::byte2bit(BYTE byte[], BYTE bit[])
{
...
void CCryptography::bit2byte(BYTE bit[], BYTE byte[])
{
...
void CCryptography::keychange(BYTE oldkey[], BYTE newkey[][8])
{
int i=0,j=0,k=0 ;
...
void CCryptography::s_replace(BYTE s_bit[])
{
int p[32] = {   16,7,20,21,
		29,12,28,17,
		1,15,23,26,
...
BYTE s_byte[64] ;
BYTE s_byte1[64] ;
BYTE s_bit_temp[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0} ;
BYTE row = 0 , col = 0 ;
BYTE s_out_bit[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0} ;
BYTE s_out_bit1[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0} ;
...
void CCryptography::DES(BYTE m_bit[], BYTE k_bit[], BYTE e_bit[])
{
...
BYTE m_bit1[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0} ;
BYTE m_byte[64] ;
BYTE m_byte1[64] ;
BYTE key_n[16][8] ;
BYTE l_bit[17][8] ;
...
void CCryptography::UnDES(BYTE m_bit[], BYTE k_bit[], BYTE e_bit[])
{
...
void CCryptography::TriDES(BYTE In[],BYTE K[],BYTE Out[])
{
BYTE Temp[8];
DES(In,K,Temp);
UnDES(Temp,&K[8],Out);
...
void CCryptography::UnTriDES(BYTE In[],BYTE K[],BYTE Out[])
{
BYTE Temp[8];
UnDES(In,K,Temp);
DES(Temp,&K[8],Out);
...
void CCryptography::EncodeDES(BYTE InData[] , BYTE Key[] , BYTE OutData[])
{
	DES(InData,Key,OutData);
}

void CCryptography::DecodeDES(BYTE InData[] , BYTE Key[] , BYTE OutData[])
{
	UnDES(InData,Key,OutData);
}

void CCryptography::Encode3DES(BYTE InData[] , BYTE Key[] , BYTE OutData[])
{
	BYTE tmp[8];
	DES(InData,Key,OutData);
	UnDES(OutData,Key+8,tmp);
	DES(tmp,Key+16,OutData);	
}

void CCryptography::Decode3DES(BYTE InData[] , BYTE Key[] , BYTE OutData[])
{
	/*
	char output[256];
	...
	for(int m=0;m<33;m++)
	{
	sprintf(output,"%02x",InData[m]);
//	::AfxMessageBox(output,NULL,MB_OK);
	}

	memset(output,0x00,sizeof(output));
	for(m=0;m<33;m++)
	{
	sprintf(output,"%02x",Key[m]);
//	::AfxMessageBox(output,NULL,MB_OK);
	}
	*/
	BYTE tmp[8];
	UnDES(InData,Key+16,OutData);
	DES(OutData,Key+8,tmp);
	UnDES(tmp,Key+0,OutData);
}


void CCryptography::DESDB(CString &ID)
{
	CString key;
//	key.LoadString(IDS_DES_KEY);
	char DES_ID[33];
	BYTE KK[8] = {'1','2','3','4','a','b','c','d'};
	BYTE newKey[8];
	BYTE data[16];
	BYTE newID[16];
	for(int i = 0;i < 8;i++)
	...
	DES(data,newKey,newID);
	DES(&data[8],newKey,&newID[8]);

	for(i = 0;i < 16;i++)
	{
		DES_ID[i*2] = (newID[i] & 0x0f) + 0x41;
		DES_ID[i*2 + 1] = ((newID[i] & 0xf0) >> 4) + 0x41;
	}
	DES_ID[32] = 0;
	ID = DES_ID;
}

void CCryptography::UnDESDB(CString &ID)
{
	CString key;
//	key.LoadString(IDS_DES_KEY);
	BYTE KK[8] = {'1','2','3','4','a','b','c','d'};
	...
	for(i = 0;i < 16;i++)
	{
		data[i] = (ID[i*2] - 0x41) + ((ID[i*2+1] - 0x41)<<4) ;
	}
	UnDES(data,newKey,newID);
	UnDES(&data[8],newKey,&newID[8]);
	
	newID[16] = 0;
	ID = newID;
	if(ID.Find(' ') > 0)
		ID.Delete(ID.Find(' '),ID.GetLength() - ID.Find(' '));
}
